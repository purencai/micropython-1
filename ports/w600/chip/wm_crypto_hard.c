...
static int16_t pstm_get_bit (pstm_int * a, int16_t idx)
{
	int16_t     r;
	int16_t n = idx / DIGIT_BIT;
	int16_t m = idx % DIGIT_BIT;
...
static int rsaMulModRead(unsigned char w, pstm_int * a)
{
	...
	//if ((err = mp_init_for_read_unsigned_bin(a, RSAN * sizeof(u32) + sizeof(pstm_int))) != ERR_CRY_OK){
	//	return err;
	//}
	if ((err = mp_read_unsigned_bin(a, (unsigned char *)in, RSAN * sizeof(u32))) != ERR_CRY_OK) {
		mp_clear(a);
		return err;
	}
	return 0;
...
static void rsaMulModWrite(unsigned char w, pstm_int * a)
{
	u32 in[64];
	memset(in, 0, 64 * sizeof(u32));
	mp_to_unsigned_bin_nr(a, (unsigned char *)in);
...
int tls_crypto_exptmod(pstm_int *a, pstm_int *e, pstm_int *n, pstm_int *res)
{
	...
	mp_init(&Y);
	mp_init(&R);
	k = mp_count_bits(n);//n->used * DIGIT_BIT;//pstm_count_bits(n);
	k = ((k / 32) + (k % 32 > 0 ? 1 : 0)) * 32;
	mp_set(&Y, k);
	mp_set(&X, 2);
	mp_exptmod(&X, &Y, n, &R); //R = 2^k % n
	//mp_set(&Y, 1);
	mp_mulmod(NULL, a, &R, n, &X); //X = A * R
	mp_copy(&R, &Y);
	if(n->used > 1)
...
int rsaMontExptMod(pstm_int *a, pstm_int *e, pstm_int *n, pstm_int *res)
{
	...
	mp_init(&Y);
	mp_init(&R);
	k = n->used * DIGIT_BIT;//mp_count_bits(n);
	mp_set(&Y, k);
	mp_set(&X, 2);
	mp_exptmod(&X, &Y, n, &R); //R = 2^k % n
	//mp_set(&Y, 1);
...
m[63] = 0xb10abd76;
	...
	mp_reverse(a, len * sizeof(u32));
	mp_reverse(b, len * sizeof(u32));
	mp_reverse(m, len * sizeof(u32));
	if ((err = mp_init_for_read_unsigned_bin(&pa, len * sizeof(u32))) != PS_SUCCESS){
		goto out;
	}
	if ((err = mp_read_unsigned_bin(&pa, a, len * sizeof(u32))) != PS_SUCCESS) {
		goto out;
	}
	if ((err = mp_init_for_read_unsigned_bin(&pb, len * sizeof(u32))) != PS_SUCCESS){
		goto out;
	}
	if ((err = mp_read_unsigned_bin(&pb, b, len * sizeof(u32))) != PS_SUCCESS) {
		goto out;
	}
	if ((err = mp_init_for_read_unsigned_bin(&pm, len * sizeof(u32))) != PS_SUCCESS){
		goto out;
	}
	if ((err = mp_read_unsigned_bin(&pm, m, len * sizeof(u32))) != PS_SUCCESS) {
		goto out;
	}
	mp_init(&pres);
	mp_init(&mres);
	...
	mp_exptmod(&pa, &pb, &pm, &mres);
	if(mp_cmp(&mres, &pres) != PSTM_EQ)
	{
		err = -1;
		goto out;
	}
